...
│   ├── layout.tsx                 # Root layout (Navbar, Footer); next/font variables sit on <html>, ground on <body>
│   ├── tokens.css                 # THE design tokens (#122) — every --dokum-* colour/type/space/motion value, declared once
│   ├── globals.css                # Tailwind entry; imports tokens.css, keyframes, the MathJax a11y shim
...
**Every colour, type step, space, duration and easing in the product is declared once, in [src/app/tokens.css](src/app/tokens.css), as a `--dokum-*` custom property cited to the ticket that decided it** (map [#112](https://github.com/dokumtastisch/dokum/issues/112): #116 ground and grain, #117 type, #118 colour and the grammar of state, #119 page/measure/space/motion/states). A hard-coded hex, duration or measure anywhere else is a bug, not a shortcut.

- **One declaration site, three consumers.** `globals.css` imports `tokens.css`, and the root layout loads `globals.css` on every route — so `:root` is already populated wherever [interactive-document.css](src/components/documents/interactive-document.css) or [editor.css](src/app/admin/editor/editor.css) renders. **Neither of them may import `tokens.css`**: a second import emits the whole set twice and gives a value two places to change.
- **Colours, and only colours, are aliased into Tailwind** via `@theme inline` (`bg-surface`, `text-ink-muted`, `border-hairline`, `border-edge`, `text-brand`). `inline` keeps the literal in `:root`. Spacing and duration are deliberately **not** wired into Tailwind's `--spacing-*` / `--ease-*` namespaces — #119 froze the spacing scale rather than re-basing it, so no existing `p-4` may change meaning underneath the product.
- **Semantic names only** — `--dokum-surface`, never `--dokum-white`. Dark mode is out of scope, and this naming is what keeps reversing that cheap.
- **One focus ring, declared once** as a bare `:focus-visible` in `tokens.css`, ink and never red. A per-component `focus:outline-none` **deletes** it rather than overriding it — don't add one. The admin forms still carry 21 of them plus 3 in the editor; they go with the admin token sweep.
- **The ground is `.dokum-ground`** (paper colour + the generated grain tile), worn by `<body>` and by the sticky navbar so the grain never stops mid-screen. The tile is `public/paper-grain.png`, regenerated by `scripts/generate-grain.py`; #118's contrast ratios are measured against the *grained darkest* ground, so re-tuning the grain invalidates them.
- **`prefers-reduced-motion` lives with the tokens**, re-pointing every duration to `1ms` (never `0s` — that can suppress `transitionend`, which `DocumentOverlay` may key off) and stopping `.animate-pulse` / `.animate-slide-up`. SC 2.2.2 is Level A and the only item here with real BFSG exposure.
